const start_default = {
    start_default: async function () {
        ...
        $.each(res_data_default['user'],function(i,v){
            html_data_sale += `<option real_data="${v['ID']}" data_name="${v['first_name']} ${v['last_name']}">${v['first_name']} ${v['last_name']}</option>`
        })

        $('.data_list_sale_list').append(html_data_sale)
        $('.data_list_cs_list').append(html_data_sale)
