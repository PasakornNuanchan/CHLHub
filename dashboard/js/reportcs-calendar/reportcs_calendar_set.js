const reportcs_calendar_set = {
    ...
    lunch_page_header : async function(){
        $('.head-of-menu').html('')
        $('.head-of-menu').append('Report Customer Service Calendar Schedule')
    ...
    runtime_lunch: async function () {
        ...
        $.each(u_m, function (i, v) {
            ...
            html_btn_m += `
            <button class="btn btn-sm btn-primary" onclick="reportcs_calendar_set.select_mounth('${day}','${day_y}')">${day}</button>`;
        ...
        $('.btnmz').append(`<button class="btn btn-sm btn-primary " onclick="reportcs_calendar_set.select_day('dy')">Day</button>
        <button class="btn btn-sm btn-primary " onclick="reportcs_calendar_set.select_type('wk')">Week</button>
        <button class="btn btn-sm btn-primary " onclick="reportcs_calendar_set.select_mounth('${date_start_t}','${date_start_y}')">Month</button>`)
    ...
    ajax_get_data_calendar: function (gen_date_start, gen_date_end) {
        return new Promise(function (resolve, reject) {
            $.ajax({
                type: "post",
                url: "php/reportcs_calendar/calendar_detail.php",
                data: {
    ...
    select_mounth: async function (date_start_t, date_start_y) {
        ...
        await $.each(arr_time, async function (i, v) {
            ...
            var daym = await reportcs_calendar_set.change_month_to_number(dayt)
        ...
        await $.each(res_calandar_task['calendar'], function (i, v) {
            ...
            html_data = `<button class="btn btn-sm btn-outline-primary col-xl-12 mt-1 bic" onclick="reportcs_calendar_set.opentabcus('${job_number}')">${type_work + ' : ' + job_number + ' (' + consignee_name + ')'}</button><br>`
            ...
            if ($(`.${d_time + m_time + y_time + 'text'} > .bic`).length < 5) {
                ...
                if ($(`.${d_time + m_time + y_time + 'text'} > .bsm`).length < 1) {
                    $(`.${d_time + m_time + y_time + 'text'}`).append(`<button class="btn btn-sm btn-block bsm" onclick="reportcs_calendar_set.modal_show_see_more(${d_time + m_time + y_time})">see more</button>`)
                }
            }
        })
    ...
    modal_show_see_more: async function (data_val) {
        ...
        $.each(res_calandar_task['calendar'], function (i, v) {
            ...
            if (data_val == d_time + m_time + y_time) {
                html_data_see += `<button class="btn btn-sm btn-outline-primary mt-1" onclick="reportcs_calendar_set.opentabcus('${job_number}')">${type_work + ' : ' + job_number + ' (' + consignee_name + ')'}</button><br>`;
            }
    ...
    select_day: async function () {
        ...
        await $.each(res_calandar_task['calendar'], function (i, v) {
            ...
            html_data = `<button class="btn btn-sm btn-sm-lg btn-outline-primary col-xl-3 mt-1 bic" onclick="reportcs_calendar_set.opentabcus('${job_number}')">${type_work + ' : ' + job_number + ' (' + consignee_name + ')'}</button><br>`
    ...
    select_type: async function (data_get) {
        ...
        let arr_week_obj = {}
        let arr_week = []
        reportcs_calendar_set.global_arr_week = arr_week;
        let round_today = 0;

        $('.btnmr').append(`<button class="btn btn-sm btn-primary" onclick="reportcs_calendar_set.select_week('p')">previous</button>
            <button class="btn btn-sm btn-primary" onclick="reportcs_calendar_set.select_week('n')">next</button>`)
        ...
        $.each(arr_week, async function (i, v) {
            if (v['round_today'] == '1') {
                ...
                reportcs_calendar_set.global_week_set = i
            }
        })
    ...
    select_week: async function (val) {
        ...
        $('.p_data > tbody').html('')
        if (val == 'p') {
            reportcs_calendar_set.global_week_set--
        } else if (val == 'n') {
            reportcs_calendar_set.global_week_set++
        }
        ...
        let arr_week = reportcs_calendar_set.global_arr_week
        $('.h_mounth').html('')

        $('.h_mounth').append(arr_week[reportcs_calendar_set.global_week_set]['month'] + ' ' + arr_week[reportcs_calendar_set.global_week_set]['year'])
        $('.p_data > tbody').append(arr_week[reportcs_calendar_set.global_week_set]['htmltbo'])
