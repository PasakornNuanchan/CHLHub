...
HAVING pct.return_payment_by = 0
    ...
    ";
    $sql_awt = "
    SELECT act.advance_cash_number,SUM(acd.amount) as total_amount ,IF(COUNT(acd.job_number) = COUNT(jt.clearlance_date),'Pass','Not Pass') as clearance_status  ,act.tranfer_by FROM advance_cash_title  as act
...
HAVING act.tranfer_by IS NULL

    ";
    $sql_hnc = "
    SELECT cp.job_number,cp.amount,IF(jt.clearlance_date IS NULL ,'Not Pass','Pass') as clearlance_status FROM cash_payment as cp
