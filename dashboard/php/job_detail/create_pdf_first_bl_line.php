...
$pdf->SetX(10);
$pdf->SetFont('times', 'B', 8, '', true);
$pdf->Cell(50, 4, strtoupper("vessel"), "TR", 0, 'L');
$pdf->SetX(60);
...
$pdf->SetX(110);
$pdf->Cell(90, 4, strtoupper("Final Destination(merchan's reference only)"), 'LT', 1, 'L');
//$pdf->Ln();
...
foreach ($data_shipperandconsignee as $k => $v) {
  ...
  $pdf->SetX(60);
  $pdf->Cell(50, 4, strtoupper($v['a4location'] . "," . $v['a4provice']), 'R', 1, 'L');
}
...
$pdf->Ln();
$pdf->Cell(38, 4, "Frieight charges on ", 'RB');
$pdf->SetX(48);
...
$pdf->Cell(35, 4, "", 'R', '', 'C');
$pdf->SetX(113);
$pdf->Cell(60, 4, "noted the total number of container or other package or units", 'L', 1, 'L');
...
$pdf->Cell(35, 4, "", 'R', '', 'C');
$pdf->SetX(113);
$pdf->Cell(60, 4, "One og the original Bills of lading must be surrended duly endorsed", 'L', 1, 'L');
...
foreach ($data_bl_list as $k => $v) {
  ...
  $pdf->Cell(25, 4, strtoupper($v['package']), "R", 0, 'C');

  $pdf->SetXY(155, $get_y_table);
  $pdf->Cell(25, 4, strtoupper($v['gross_weight']), "R", 0, 'C');

  $pdf->SetXY(175, $get_y_table);
  $pdf->Cell(30, 4, strtoupper($v['measurement']), "R", 0, 'C');

  $pdf->SetXY(100, $get_y_table);
  $pdf->MultiCell(55, 4, strtoupper($v['kind_of_package']), "R", 'L');
...
foreach ($container_data as $k => $v) {
  $pdf->SetX(15);
  $pdf->MultiCell(150, 4, $v['container_number']."/".$v['seal_number'] . "/" . $v['container_type'] . "/" . $v['quantity'] . $v['unit'] . "/" . $v['gw'] . ".KGS/" . $v['cbm'] . "CBM", 0, 'L');
}
...
$pdf->Cell(90, 4, "Anthorized Signature", '', '', 'C');
...
foreach ($data as $k => $v) {
  if ($v['bill_header'] == "CHL") {
    ...
    $pdf->SetXY(113, 240);
    $pdf->SetFont('times', 'B', 12, '', true);
    $pdf->Cell(90, 4, "CHINA HIGNWIN(THAILAND) LIMIED", '', 1, 'C');
  } else {
...
// $pdf->SetXY(10, 15);
// foreach ($data_shipperandconsignee as $k => $v) {
//   $pdf->Cell(180, 6, "ATTACHED SHEET FOR B/L NO.:" . $v['mbl'] . " VESSEL/VOY : " . $v['mother_vessel'], 0, 1, "C");
// }
...
// $pdf->SetXY(170, 10);
// $pdf->SetFont('times', 'B', 9, '', true);
// $page_last = $pdf->PageNo()-1;
// $pdf->Cell(50, 5, "Page ".$page_last." of ".$page_last);





// foreach($data_description as $data_description){

// }
// foreach ($data_description as $data_description) {
//     $width = $pdf->GetStringWidth($data_description);
//     if ($pdf->GetY() + 10 > 290 || $width > 180) { // 290 is the Y-coordinate limit for the page
//         $pdf->AddPage();
//     }
//     $pdf->MultiCell(0, 10, $data_description);
// }
