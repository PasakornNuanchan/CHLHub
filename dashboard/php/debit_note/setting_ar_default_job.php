...
WHERE b.type ='AR' AND b.status = '0' $cs_support $job_number $q_date $q_bill_to
ORDER BY bill_to_c
$limit_data
";
...
if ($result->num_rows > 0) {
  while ($row = $result->fetch_assoc()) {
    $data_setting_default_ar_by_job[] = $row;
    //$data_setting_default_ar_by_job[] = $row['ID'];
  }
  foreach($data_setting_default_ar_by_job as $k => $v){
    $data_setdefault_ar_by_job[$v['ID']][] = $v;
  }
} else {
  $data_setdefault_ar_by_job = "0 results";
}

// foreach($data_setting_default_ar_by_job as $k => $v){
//     $data_setting_default_ar_by_job[] = 
// }


echo json_encode(array('data_setdefault_ar_by_job'=>$data_setdefault_ar_by_job))
